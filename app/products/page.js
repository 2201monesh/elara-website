...
const ProductsView = () => {
  ...
  const [allProducts, setAllProducts] = useState(fetchAllProducts());
  const [searchInput, setSearchInput] = useState("");
  ...
  return (
    <div className="products-view">
      <div className="my-16 mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
        <h1 className="leading-snug tracking-tight font-semibold text-3xl sm:text-5xl">
          Locks & Accessories by Elara
        </h1>
        <p className="text-lg text-brown mt-4">
          We have something for everyone, from locks to door handles, and
        ...
        <div className="products-list-container">
          <div className="product-recommendations-options flex flex-row flex-wrap items-center justify-start gap-6 my-6">
            {allCategories.map((categoryItem, categoryIndex) => (
              <button
                key={categoryIndex}
                onClick={() => handleCategoryFilter(categoryItem)}
                className={category === categoryItem ? 'active' : ''}
        ...
        <div className="product-items-container grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mt-8">
          {allProducts.map((product, index) => (
            <ProductCard
              title={product.title}
              description={product.description}
